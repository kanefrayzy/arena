...
export class PixiRenderer {
  ...
  private tick(dtMs: number): void {
    ...
    // Linear interpolation between the last two server snapshots.
    // Snapshots arrive every ~33 ms; t is clamped to [0..1] so the sprite
    // holds at the latest authoritative position when the next snapshot is
